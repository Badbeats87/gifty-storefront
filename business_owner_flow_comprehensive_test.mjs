...
// STEP 2: Get a visible business with data
console.log('STEP 2️⃣  - SELECT BUSINESS');
console.log('─'.repeat(65));

const { data: businesses } = await supabase
  .from('businesses')
  .select('id, name, contact_name, contact_email, created_at, is_visible')
  .eq('is_visible', true)
  .limit(1);

if (!businesses || businesses.length === 0) {
  console.log('❌ No visible businesses found in database\n');
  console.log('💡 Note: Admin must make businesses visible before business owners can access portal\n');
  process.exit(1);
}
...
if (giftCards && giftCards.length > 0) {
  ...
  giftCards?.slice(0, 5).forEach((card, i) => {
    ...
    console.log(`   ${i+1}. ${statusEmoji} ${card.id.substring(0, 12)}...`);
...
console.log('─'.repeat(65));
const checks = [
  { name: 'Business is visible to owner', result: business.is_visible },
  { name: 'Business has correct email', result: !!business.contact_email },
  { name: 'Gift cards exist for business', result: (giftCards?.length || 0) > 0 },
  { name: 'Orders exist for business', result: (orders?.length || 0) > 0 },
  { name: 'Gift card business_id integrity', result: giftCards?.every(c => c.business_id === businessId) },
  { name: 'Order business_id integrity', result: orders?.every(o => o.business_id === businessId) },
];
...
if (passCount === checks.length) {
  console.log('🎉 ALL BUSINESS OWNER FLOW TESTS PASSED!\n');
} else {
  console.log('⚠️  Some checks failed. Review the data above.\n');
}
